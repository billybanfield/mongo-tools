...
type OpStreamSettings struct {
	...
	MaxBufferedPages int    `long:"maxBufferedPages" description:"maximum number of pages to store when buffering packets"`
}
...
func (bidi *bidi) handleStreamCompleted() {
	...
	if !lastOpTimeStamp.IsZero() {
		bidi.opStream.unorderedOps <- RecordedOp{
			Seen:              lastOpTimeStamp.Add(time.Nanosecond),
			SeenConnectionNum: bidi.connectionNumber,
			EOF:               true,
		}
	}
